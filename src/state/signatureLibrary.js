...
export const useSignatureLibrary = create()(persist((set) => ({
    signatures: [],
    addSignature: (input) => {
        const now = Date.now();
        const entry = {
    ...
    renameSignature: (id, label) => set((state) => ({
        signatures: state.signatures.map((signature) => signature.id === id ? { ...signature, label: normalizeLabel(label) } : signature),
    })),
    deleteSignature: (id) => set((state) => ({
        signatures: state.signatures.filter((signature) => signature.id !== id),
    })),
    markUsed: (id) => set((state) => ({
        signatures: state.signatures.map((signature) => signature.id === id ? { ...signature, lastUsedAt: Date.now() } : signature),
    })),
