...
const loadSourceDocument = async (pdf: LoadedPdf) => {
  try {
...
export const extractPagesFromLoadedPdf = async (
  pdf: LoadedPdf,
  pageNumbers: number[],
): Promise<Uint8Array> => {
...
export const buildZipFromEntries = async (entries: ZipEntry[]): Promise<ArrayBuffer> => {
  if (entries.length === 0) {
