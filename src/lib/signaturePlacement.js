...
export const moveTextPlacement = ({ placement, deltaXPct, deltaYPct }) =>
  clampTextWithinCanvas({
    ...placement,
    xPct: placement.xPct + deltaXPct,
    yPct: placement.yPct + deltaYPct,
  });
export const resizeTextPlacement = ({ placement, nextWidthPct }) =>
  clampTextWithinCanvas({
    ...placement,
    widthPct: nextWidthPct,
  });
...
export const strokeToPdfPoints = (stroke, size) =>
  stroke.points.map((p) => ({
    x: p.xPct * size.width,
