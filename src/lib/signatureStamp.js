...
import { dataUrlToUint8Array, placementToPdfRect, textPlacementToPdfPosition, } from "./signaturePlacement";
const clampPageIndex = (pageNumber, pageCount) => {
...
const getPageSize = (cache, pageIndex, page) => {
...
const renderStrokesToPng = (strokes, pageSize) => {
    if (strokes.length === 0)
        return null;
...
export const stampSignaturesToExportResult = async ({ pdf, placements, signatures, textPlacements = [], strokes = [], strokesOnTop = true, startedAt = Date.now(), }) => {
    ...
    if (appliedTextPlacements > 0) {
        detailParts.push(`${appliedTextPlacements} text block${appliedTextPlacements === 1 ? "" : "s"}`);
    }
    if (appliedStrokes > 0) {
