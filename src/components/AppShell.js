...
const AppShell = () => {
    ...
    return (_jsx("div", { className: "min-h-screen bg-slate-50 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100", children: _jsxs("div", { className: "mx-auto flex max-w-6xl flex-col gap-8 px-4 py-8 lg:flex-row lg:py-12 2xl:max-w-7xl", children: [_jsxs("aside", { className: "rounded-3xl border border-slate-200/60 bg-white/80 p-6 shadow-2xl shadow-slate-200/40 backdrop-blur dark:border-white/10 dark:bg-slate-900/70 dark:shadow-slate-900/40 lg:sticky lg:top-8 lg:h-fit lg:w-72 2xl:w-80", children: [_jsxs("div", { className: "flex items-center justify-between gap-4", children: [_jsxs(Link, { to: "/", className: "flex items-center gap-3 group", children: [_jsx("img", { src: logoSrc, alt: "PDF Workbench logo", className: "h-10 w-10 rounded-xl" }), _jsxs("div", { children: [_jsx("p", { className: "text-xs uppercase tracking-[0.25em] text-slate-500 group-hover:text-slate-700 dark:text-slate-400 dark:group-hover:text-slate-200 transition-colors", children: "PDF WORKBENCH" }), _jsx("p", { className: "font-display text-2xl font-semibold text-slate-900 dark:text-white", children: "v0.6 Compression" })] })] }), _jsxs("div", { className: "flex gap-2", children: [_jsx(ThemeToggle, {}), _jsxs("button", { type: "button", className: "inline-flex rounded-full border border-slate-200/70 bg-white p-2 text-slate-600 shadow hover:border-slate-300 hover:text-slate-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand dark:border-white/20 dark:bg-slate-800 dark:text-slate-200 lg:hidden", "aria-label": "Toggle navigation", "aria-expanded": navOpen, onClick: handleNavToggle, children: [_jsxs("span", { className: "sr-only", children: [navOpen ? "Close" : "Open", " navigation"] }), navOpen ? "✕" : "☰"] })] })] }), _jsxs("nav", { className: clsx("mt-8 space-y-2 text-sm font-medium", navOpen ? "block" : "hidden lg:block", isAnimating && "animate-pulse"), children: [_jsxs(NavLink, { to: "/", end: true, className: ({ isActive }) => clsx("flex items-center justify-between rounded-2xl px-4 py-3 transition", isActive
